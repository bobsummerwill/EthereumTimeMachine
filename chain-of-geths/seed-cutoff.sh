...
sudo docker compose stop geth-v1-10-0 geth-v1-9-25 geth-v1-3-6 >/dev/null 2>&1 || \
  sudo docker-compose stop geth-v1-10-0 geth-v1-9-25 geth-v1-3-6 >/dev/null 2>&1 || true
...
import_one v1.10.0 ethereumtimemachine/geth:v1.10.0
...
sudo docker compose up -d geth-v1-10-0 geth-v1-9-25 geth-v1-3-6 >/dev/null 2>&1 || \
  sudo docker-compose up -d geth-v1-10-0 geth-v1-9-25 geth-v1-3-6 >/dev/null 2>&1

echo "Seed complete. Quick sanity check (eth_blockNumber):"
for name_port in "v1.10.0:8551" "v1.9.25:8552" "v1.3.6:8549"; do
